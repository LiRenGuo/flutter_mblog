...
class _EditMinePageState extends State<EditMinePage> {
      ...
      child: WillPopScope(
                            ...
                            children: <Widget>[
                              ...
                              InkWell(
                                child: Container(
                                  height: AdaptiveTools.setPx(80),
                                  padding:
                                      EdgeInsets.all(AdaptiveTools.setPx(30)),
                                  decoration: BoxDecoration(
                                      color: Colors.black38,
        ...
        onWillPop: () async {
          if(!onUpdate){
            ...
            return await showConfirmExitDialog(context);
          }
        },
      ),
    );
  ...
  _takePhoto(String flag) async {
    ...
    if (flag == "banner") {
      banner =
          await UploadDao.uploadImage(UploadDao.UPDATE_BANNER, image, context);
    } else if (flag == "avatar") {
      avatar =
          await UploadDao.uploadImage(UploadDao.UPDATE_BANNER, image, context);
    }
